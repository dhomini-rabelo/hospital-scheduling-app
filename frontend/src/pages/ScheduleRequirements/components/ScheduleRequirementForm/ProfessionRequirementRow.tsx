...
export function ProfessionRequirementRow({
  ...
  const profession = useWatch<CreateScheduleRequirementSchema>({
  ...
  const {
    ...
    control,
    name: `requirements.${index}.specialtyRequirements`,
  })
  ...
  useEffect(() => {
    if (profession) {
      setValue(`requirements.${index}.specialtyRequirements`, [])
    }
  }, [profession, index, setValue])

  const fieldErrors = formState.errors.requirements?.[index]
      ...
      <div className="flex items-start gap-3">
        <div className="grid flex-1 grid-cols-2 gap-3">
          <Select
            ...
            registration={register(`requirements.${index}.profession`)}
          ...
          <Input
            ...
            placeholder="e.g. 5"
            min={1}
            registration={register(`requirements.${index}.requiredCount`, { valueAsNumber: true })}
